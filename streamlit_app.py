...
with col1:
    issue_filter_raw = st.selectbox(
        "Annotation Status",
        options = new_df.Issue.unique(),
        format_func=lambda x: "Annotated" if x else "Pending"
    )
with col2:
    category_options = new_df[new_df["Issue"]==issue_filter_raw].Category.unique()
    category_filter = st.selectbox("Choose a category", options = category_options)

filtered_df = new_df[(new_df['Issue'] == issue_filter_raw) & (new_df['Category'] == category_filter)]
