...
with col1:
    issue_filter = st.selectbox(
        ...
        help="Filter the annotated data based on whether an issue was identified."
    )
with col2:
    category_filter = st.selectbox(
        "Choose a category",
        options=new_df[new_df["Issue"] == issue_filter].Category.unique(),
        help="Select a specific issue category to filter the results."
    )
...
with col1:
    st.metric(
        ...
        issue_cnt,
        help="Total number of model responses marked with at least one issue."
    )
with col2:
    st.metric(
        "Annotation Progress",
