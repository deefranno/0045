    ...
    <div style={{ paddingBottom: '80px' }}>
      ...
      <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        <section className="card">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '8px' }}>
            {['JMD', 'USD', 'TTD', 'BBD'].map(curr => (
              <button key={curr} type="button" onClick={() => setFormData({ ...formData, currency: curr })} style={{ padding: '10px', borderRadius: '10px', border: '1px solid var(--border)', background: formData.currency === curr ? 'var(--brand)' : 'white', color: formData.currency === curr ? 'white' : 'inherit', fontWeight: '600' }}>
                {curr}
              </button>
            ))}
          </div>
        ...
        <section>
          ...
          {formData.items.map((item, index) => (
            <div key={index} className="card" style={{ marginBottom: '12px', background: '#F2F7F5' }}>
              <div style={{ display: 'flex', gap: '8px', marginBottom: '8px' }}>
                ...
                <button type="button" onClick={() => removeItem(index)} style={{ background: 'none', border: 'none', color: 'var(--danger)' }}><Trash2 size={20} /></button>
              </div>
          ...
          <button type="button" onClick={addItem} className="btn-ghost" style={{ borderStyle: 'dashed', color: 'var(--brand)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
