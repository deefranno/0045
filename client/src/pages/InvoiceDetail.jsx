...
import { generateMessage, openWhatsApp } from '../utils/whatsapp';
...
const InvoiceDetail = () => {
  ...
  const handleWhatsApp = () => {
    const msg = generateMessage(invoice, invoice, 'QuickInvoice Demo', invoice.status === 'paid' ? 'paid_receipt' : 'reminder');
    openWhatsApp(invoice.client_phone, msg);
  };
  ...
  return (
    <div>
      ...
      <div style={{ padding: '24px 16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
        <button onClick={handleWhatsApp} className="btn-primary" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
          <MessageSquare size={18} /> {invoice.status === 'paid' ? 'Send Receipt' : 'Remind via WhatsApp'}
        </button>
      ...
      {/* Hidden Print Area */}
      <div className="invoice-print-area" style={{ display: 'none', padding: '40px', color: 'black' }}>
        <div style={{ textAlign: 'center', marginBottom: '40px' }}>
          <h1 style={{ color: '#0F6E56', margin: 0 }}>INVOICE</h1>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '40px' }}>
          <div>
            ...
            <p style={{ margin: 0, color: '#666' }}>{invoice.client_address}</p>
          </div>
          <div style={{ textAlign: 'right' }}>
            <p style={{ margin: 0 }}><strong>Date:</strong> {invoice.issue_date}</p>
          ...
          <tbody>
            {invoice.items.map((item, i) => (
              <tr key={i} style={{ borderBottom: '1px solid #eee' }}>
                <td style={{ padding: '12px' }}>{item.description} ({item.quantity}x)</td>
                <td style={{ padding: '12px', textAlign: 'right' }}>${(item.unit_price * item.quantity).toLocaleString()}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div style={{ textAlign: 'right' }}>
          <p style={{ margin: '0 0 8px 0' }}>Subtotal: ${invoice.subtotal.toLocaleString()}</p>
          {invoice.gct_enabled === 1 && <p style={{ margin: '0 0 8px 0' }}>GCT (15%): ${(invoice.subtotal * 0.15).toLocaleString()}</p>}
          <h2 style={{ color: '#1D9E75' }}>Total: {invoice.currency} ${invoice.total.toLocaleString()}</h2>
        </div>
      </div>
    </div>
  );
